...
;(function($) {
  ...
  $sk.fn.extend({
    ...
    nextImage: function() 
    {
      ...
      switch (animation_type) 
      {
        ...
        case 'cubeSize' : 
          ...
          break;
		case 'cubeSizeToRight' :
		  this.animationCubeSizeToRight();
		  break;
        case 'horizontal' : 
        ...
        case 'showBarsRandom' : 
          ...
          break;
        case 'tube' : //numbers:10 by default
          this.animationTube();
          break;
		case 'tube1' :
			this.animationTube({numbers:1});
			break;
		case 'tube3' :
			this.animationTube({numbers:3});
			break;
		case 'tube7' :
			this.animationTube({numbers:7});
			break;	
        case 'fade' : 
          this.animationFade();
          break;
        ...
        case 'blindWidth' : 
          ...
          break;
        case 'directionTop' : 
          this.animationDirection({direction:'top', total:5});
          break;
        case 'directionBottom' : 
          this.animationDirection({direction:'bottom', total:5});
          break;
        case 'directionRight' : 
    ...
    animationCubeSizeToRight: function(options)
	{
		var self = this;
		...
		//this.box_skitter.find('.image_main').attr({'src':this.settings.image_atual});
		//this.box_skitter.find('.image_main').css('opacity','1');// fixed image previos
		//var division_w 	= Math.ceil(this.settings.width_skitter / (this.settings.width_skitter / 15));
		//var division_h 	= Math.ceil(this.settings.height_skitter / (this.settings.height_skitter / 1));
		...
		for (i = 0; i < total; i++) {
			...
			var box_clone = this.getBoxCloneImgOld(image_old);
			box_clone.css({left:_vleft, top:_vtop, width:width_box, height:height_box});
			box_clone.find('img').css({left:_vleft_image, top:_vtop_image,width:this.settings.width_skitter, height:this.settings.height_skitter});
			
			this.addBoxClone(box_clone);
			box_clone.show();
			
			var delay_time = 150 * i; // [gene] 50 change to 150
			var callback = (i == (total - 1)) ? function() { self.finishAnimation(); } : '';
			box_clone.delay(delay_time).animate({
				opacity:'hide',width:'hide',height:'hide',top:_vtop+(width_box*1.5),left:_vleft+(height_box*1.5)
			}, time_animate, easing, callback);
			...
			if(i == (total - 1))
			{
				...
				this.box_skitter.find('.image_main').attr({'src':this.settings.image_atual});
				this.box_skitter.find('.box_clone').show();
				this.box_skitter.find('.image_main').css('opacity','1');// fixed image previos
    ...
    slideLeft: function(options)
	{
		...
		var total 		= Math.ceil(this.settings.width_skitter / (this.settings.width_skitter / 1));
		var width_box 	= (this.settings.width_skitter);
		var height_box 	= Math.ceil(this.settings.height_skitter / total);
		...
		for (i = 0; i < total; i++) {
			...
			box_clone.css({left:_bleft+'px', top:_btop+'px', width:width_box, height:height_box});
			box_clone.find('img').css({left:0, top:-_btop,width:this.settings.width_skitter, height:this.settings.height_skitter});
			
			this.addBoxClone(box_clone);
			
			var delay_time = 150 * i;//90;
			var callback = (i == (total - 1)) ? function() { self.finishAnimation(); } : '';
			box_clone.show();
			//box_clone.delay(delay_time).animate({opacity:'show', top:_btop, left:0}, time_animate, easing, callback);
			box_clone.delay(delay_time).animate({ top:_btop, left:0}, time_animate, easing, callback);
		}
	},
    ...
    {
	  var options = $.extend({}, {numbers:10}, options || {});
      var self = this;
      ...
      var total     = Math.ceil(this.settings.width_skitter / (this.settings.width_skitter / options.numbers));
      var width_box   = Math.ceil(this.settings.width_skitter / total);
      var height_box  = this.settings.height_skitter;
    ...
    animationCubeSpread3x3: function(options)
	{
		var self = this;
		
		this.settings.is_animating = true;
		var easing = (this.settings.easing_default == '') ? 'easeOutQuad' : this.settings.easing_default;
		var time_animate = 1200 / this.settings.velocity;//700
		
		this.setActualLevel();
		
		var division_w 	= Math.ceil(this.settings.width_skitter / (this.settings.width_skitter / 3));
		var division_h 	= Math.ceil(this.settings.height_skitter / (this.settings.width_skitter / 4));
		var total		= division_w * division_h;
		...
		for (i = 0; i < total; i++) {
			...
			var delay_time = 60 * (Math.random() * 30);//30
			if (i == (total-1)) delay_time = 60 * 30; // 30
			var callback = (i == (total - 1)) ? function() { self.finishAnimation(); } : '';
			box_clone.delay(delay_time).animate({opacity:'show',top:_btop+'px', left:_bleft+'px'}, time_animate, easing, callback);
